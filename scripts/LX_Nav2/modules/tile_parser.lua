...
local DT_POLY_DETAIL_SIZE = 12
local DT_DETAIL_VERT_SIZE = 12
local DT_DETAIL_TRI_SIZE = 4
local DT_BV_NODE_SIZE = 16
local DT_OFF_MESH_CON_SIZE = 36
local DT_LINK_SIZE = 16
...
local function parse_vertices(data, pos, count)
    ...
    for i = 1, count do
        ...
        -- wowX = recastZ, wowY = recastX, wowZ = recastY
        vertices[i] = {
            x = rz,
            y = rx,
            z = ry
...
local function parse_polygons(data, pos, count, vertices)
    ...
    for i = 1, count do
        ...
        -- Get actual vertex positions for this polygon (worldVerts)
        poly.worldVerts = {}
...
--- Parse detail meshes (12 bytes each)
...
--- Parse detail vertices (12 bytes each: 3 floats)
...
--- Parse detail triangles (4 bytes each)
...
--- Parse off-mesh connections (36 bytes each)
...
function TileParser.parse(data)
    ...
    tile.layer = tile.meshHeader.layer
    tile.walkableClimb = tile.meshHeader.walkableClimb
...
function TileParser.parse_incremental(data, budget)
    ...
    -- Parse off-mesh connections (YIELDS periodically)
    tile.offMeshConnections = {}
    if tile.meshHeader.offMeshConCount > 0 then
        tile.offMeshConnections, pos = parse_off_mesh_connections(data, pos, tile.meshHeader.offMeshConCount)
    end
